...
class AAE_Encoder(nn.Module):
    ...
    def forward(self, x):
        x = torch.leaky_relu(self.conv1_1(x), 0.2)
        x = torch.leaky_relu(self.conv2_bn(self.conv2(x)), 0.2)
        x = torch.leaky_relu(self.conv3_bn(self.conv3(x)), 0.2)
        x = self.conv4(x)
        return torch.tanh(x)
...
class AAE_Generator(nn.Module):
    ...
    # forward method
    def forward(self, x):
        x = torch.relu(self.deconv1_1_bn(self.deconv1_1(x)))
...
class AAE_Discriminator(nn.Module):
    ...
    def forward(self, x):
        x = torch.leaky_relu((self.linear1(x)), 0.2)
        x = torch.leaky_relu((self.linear2(x)), 0.2)
