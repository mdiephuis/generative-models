...
class AAE_Encoder(nn.Module):
    def __init__(self, in_channels, latent_size, d=128):
        ...
        self.network = nn.Sequential(
            nn.Conv2d(self.in_channels, self.d // 2, 4, 2, 1),
            nn.LeakyReLU(0.2),
            nn.Conv2d(self.d // 2, self.d * 2, 4, 2, 1),
            nn.BatchNorm2d(self.d * 2),
            nn.LeakyReLU(0.2),
            nn.Conv2d(self.d * 2, self.d * 4, 4, 2, 1),
            nn.BatchNorm2d(self.d * 4),
            nn.LeakyReLU(0.2),
            nn.Conv2d(self.d * 4, self.latent_size, 4, 1, 0)
        )
...
class AAE_Generator(nn.Module):
    def __init__(self, in_channels, latent_size, d=128):
        super(AAE_Generator, self).__init__()
        self.in_channels = in_channels
        self.latent_size = latent_size
        self.d = d

        self.network = nn.Sequential(
            nn.ConvTranspose2d(self.latent_size, self.d * 2, 4, 1, 0),
            nn.BatchNorm2d(self.d * 2),
            nn.LeakyReLU(0.1),
            nn.ConvTranspose2d(self.d * 2, self.d * 2, 4, 2, 1),
            nn.BatchNorm2d(self.d * 2),
            nn.LeakyReLU(0.1),
            nn.ConvTranspose2d(self.d * 2, self.d, 4, 2, 1),
            nn.BatchNorm2d(self.d),
            nn.LeakyReLU(0.1),
            nn.ConvTranspose2d(self.d, self.in_channels, 4, 2, 1)
        )
...
class AAE_Discriminator(nn.Module):
    def __init__(self, latent_size, d=128):
        ...
        self.d = d
        self.network = nn.Sequential(
            nn.Linear(self.latent_size, self.d),
            nn.LeakyReLU(0.2),
...
class AAE_MNIST_Encoder(nn.Module):
    def __init__(self, in_channels, latent_size):
        super(AAE_MNIST_Encoder, self).__init__()
        self.in_channels = in_channels
        self.latent_size = latent_size
...
class AAE_MNIST_Generator(nn.Module):
    def __init__(self, out_channels, latent_size):
        super(AAE_MNIST_Generator, self).__init__()
        self.out_channels = out_channels
        self.latent_size = latent_size
